...
{
    [TestFixture]
    class TheRightRectangleTest
    {
        [TestCase("0", -1, 8, "y", 10, 0)]
        [TestCase("5", 0, 0, "x", 10, 0)]
        [TestCase("5", 3, 20, "x", 10, 85)]
        [TestCase("(cos(0.3 * x + 0.8))/(0.9 + 2 * sin(0.4 * x + 0.3))", 0.2, 1, "x", 10, 0.227116)]
        [TestCase("sqrt(1.7 * x^2 + 0.5)/(1.4 + sqrt(1.2 * x + 1.3))", 0.7, 2.1, "x", 10, 0.905581)]
            ...
            string integrationVariable, int nodes, double expectedResult)
        {
            ...
            TheRightRectangle.TheRightRectangleMethod(function);
            var result = TheRightRectangle.Result;
